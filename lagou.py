...
tag = ['companyName', 'companyShortName', 'positionName', 'education', 'salary', 'financeStage',
       'companySize',
...
def read_page(url, page_num, keyword):  # 模仿浏览器post需求信息，并读取返回后的页面信息
    ...
    if page_num == 1:
        ...
        payload = {'first': boo, 'pn': page_num, 'kd': keyword}
        print payload
        r = requests.get(url, params=payload)
        r.headers = page_headers

        req = requests.get(url, headers=page_headers)
        # page = requests.urlopen(req, data=page_data.encode('utf-8')).read()
...
def save_excel(fin_result, tag_name, file_name):  # 将抓取到的招聘信息存储到excel当中
    book = xlsxwriter.Workbook(r'C:\Users\shuwei.chen\Desktop\%s.xls' % file_name)  # 默认存储在桌面上
...
if __name__ == '__main__':
    ...
    a = read_page(url, 1, keyword)
    print a
    max_page_num = read_max_page(a)
